...
enum enIndicatorDominance
  {
   dual_dominance, // Dual Dominance
   dsluRsi_main, // Dslu of RSI as main 
   diNapoli_main // DiNapoli Stoch as Main
  };
//+------------------------------------------------------------------+
//| Inputs                                                           |
//+------------------------------------------------------------------+
...
input uint diNapoliInpFastK = 10; // Period of fast %K line
input uint diNapoliInpSlowK = 6; // Period of slow %K line
input uint diNapoliInpSlowD = 9; // Period of slow %D line
input int diNapoliInpShift = 0; // Shift (in bars)

input group "Lookback"
input ushort DsluRSItolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort DiNapoliStochtolerance = 5; // DiNapoili Stochastic Lookback Tolerance

input group "Dominance"
input enIndicatorDominance indiDominance = dual_dominance; // Indicator Dominance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
int ATRChannelHandle;
int DsluRSIHandle;
int DiNapoliStochHandle;
//+------------------------------------------------------------------+
//| Global Variables                                                 |
//+------------------------------------------------------------------+
int barTotal;
...
int OnInit()
  {
   ...
   DiNapoliStochHandle = iCustom(_Symbol,Timeframe,"dinapolistochastic.ex5",diNapoliInpFastK,diNapoliInpSlowK,diNapoliInpSlowD,diNapoliInpShift);
...
enTradeSignal CheckForSignal()
  {
   enConfirmationSignal ConfirmationSignal = CheckConfirmationIndicators(indiDominance);
...
//+------------------------------------------------------------------+
enConfirmationSignal CheckConfirmationIndicators(enIndicatorDominance dominance)
  {
   // Indicator Values:
   ...
   double DiNapoliSignalLineValues[];
   CopyBuffer(DiNapoliStochHandle,1,1,2,DiNapoliSignalLineValues);
   ArrayReverse(DiNapoliSignalLineValues);
   
//+-----------------------Dual Dominance----------------------------+

   if (dominance == dual_dominance)
   {
    // Dslu of RSI
    // Buy
    if ((DsluRsiColourValues[0] == 1) && ((DsluRsiColourValues[1] == 0) || (DsluRsiColourValues[1] == 2)))
    {
     if (DiNapoliSignalLineValues[0] > 50)
     {
      if (DiNapoliStochLookback(DiNapoliStochtolerance) == lookback_pass)
      {
    ...
    // DiNapoli Stochastic
    // Buy
    if ((DiNapoliSignalLineValues[0] > 50) && (DiNapoliSignalLineValues[1] < 50))
    {
...
//+------------------DiNapoli Stochastic As Main-----------------------+
   if (dominance == diNapoli_main)
   {
    // Buy
    if ((DiNapoliSignalLineValues[0] > 50) && (DiNapoliSignalLineValues[1] < 50))
    {
...
enLookbackReturn DsluRSILookback(int tolerance)
  {
   ...
   double DsluRSIColourValues[];
   CopyBuffer(DsluRSIHandle,5,1,candleLookbackCount,DsluRSIColourValues);
   ArrayReverse(DsluRSIColourValues);
...
//+------------------------------------------------------------------+
//| DiNapoli Stochastic lookback Function                            |
//+------------------------------------------------------------------+
enLookbackReturn DiNapoliStochLookback(int tolerance)
  {
   int candleLookbackCount = tolerance + 2;
   
   double DiNapoliSignalLineValues[];
   CopyBuffer(DiNapoliStochHandle,1,1,candleLookbackCount,DiNapoliSignalLineValues);
   ArrayReverse(DiNapoliSignalLineValues);
   ...
   if (DiNapoliSignalLineValues[0] > 50)
   {
    for (int candlePos = 0; candlePos <= tolerance; candlePos++)
    {
     if ((DiNapoliSignalLineValues[candlePos] > 50) && (DiNapoliSignalLineValues[candlePos + 1] < 50))
     {
      return lookback_pass;
     }
    }
