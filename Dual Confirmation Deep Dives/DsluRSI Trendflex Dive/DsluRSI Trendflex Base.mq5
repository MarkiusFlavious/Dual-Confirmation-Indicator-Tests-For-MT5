...
enum enIndicatorDominance
  {
   ...
   dsluRsi_dominance, // Dslu of RSI as Main
   trendflex_dominance // Trendflex as Main
  };
//+------------------------------------------------------------------+
//| Inputs                                                           |
//+------------------------------------------------------------------+
...
input enTrendFlexMethod trendflexMethod = twoLine_cross; // Confirmation Method
...
input enIndicatorDominance InpDominance = dual_dominance; // Indicator Dominance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   if (InpDominance == dual_dominance)
   {
    ...
    if ((DsluRSIColourValues[0] == 1) && ((DsluRSIColourValues[1] == 0) || (DsluRSIColourValues[1] == 2)))
    {
     ...
     if (trendflexMethod == zero_cross)
     {
      if (TrendflexSlowValues[0] > 0)
      {
       if (TrendflexLookback(TrendflexTolerance) == lookback_pass)
       {
        return confirmation_buy;
...
//+-------------------------------------------------------------+   
    if (trendflexMethod == zero_cross)
    {
     ...
     if ((TrendflexSlowValues[0] > 0) && (TrendflexSlowValues[1] < 0))
     {
      if (DsluRSIColourValues[0] == 1)
      {
       if (DsluRSILookback(DsluRSITolerance) == lookback_pass)
   ...
   if (InpDominance == dsluRsi_dominance)
   {
    if ((DsluRSIColourValues[0] == 1) && ((DsluRSIColourValues[1] == 0) || (DsluRSIColourValues[1] == 2)))
    {
     if (trendflexMethod == twoLine_cross)
   ...
   if (InpDominance == trendflex_dominance)
   {
    if (trendflexMethod == twoLine_cross)
    {
...
enLookbackReturn TrendflexLookback(int tolerance)
  {
   ...
   if (trendflexMethod == zero_cross)
   {
    ...
    if (TrendflexSlowValues[0] > 0)
    {
     for (int candlePos = 0; candlePos <= tolerance; candlePos++)
     {
      if ((TrendflexSlowValues[candlePos] > 0) && (TrendflexSlowValues[candlePos + 1] < 0))
      {
       return lookback_pass;
      }
     }
