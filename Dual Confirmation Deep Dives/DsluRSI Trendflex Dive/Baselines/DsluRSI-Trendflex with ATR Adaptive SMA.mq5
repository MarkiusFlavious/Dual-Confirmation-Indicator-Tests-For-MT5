...
input group "ATR Adaptive SMA Inputs"
input int inpMaPeriod = 25; // SMA period
input ENUM_APPLIED_PRICE inpPrice = PRICE_CLOSE; // Price
input enColorMode inpColorMode = col_onCross; // Color change mode
input ushort AASMA_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort AASMA_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"ATR Adaptive SMA.ex5",inpMaPeriod,inpPrice,inpColorMode);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(BaselineHandle,1,1,2,BaselineValues);
   ...
   if ((closePrice1 > BaselineValues[0]) && (closePrice2 < BaselineValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(AASMA_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(AASMA_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(BaselineHandle,1,1,1,BaselineValue);
