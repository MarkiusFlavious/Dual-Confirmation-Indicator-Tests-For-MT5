...
input group "LWMA Input"
input int inpLWMAperiod = 20; // Period
input ENUM_APPLIED_PRICE inpLWMAprice = PRICE_CLOSE; // Applied Price
input ushort LWMADsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort LWMATrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   LWMAHandle = iMA(_Symbol,Timeframe,inpLWMAperiod,0,MODE_LWMA,inpLWMAprice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(LWMAHandle,0,1,2,MovingAverageValues);
   ...
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(LWMADsluRSITolerance) == lookback_pass) && (TrendflexLookback(LWMATrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(LWMAHandle,0,1,1,MovingAverageValue);
