...
input int inpEMAperiod = 20; // Period
input ENUM_APPLIED_PRICE inpEMAprice = PRICE_CLOSE; // Applied Price
input ushort EMADsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort EMATrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   EMAHandle = iMA(_Symbol,Timeframe,inpEMAperiod,0,MODE_EMA,inpEMAprice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(EMAHandle,0,1,2,MovingAverageValues);
   ...
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(EMADsluRSITolerance) == lookback_pass) && (TrendflexLookback(EMATrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(EMAHandle,0,1,1,MovingAverageValue);
