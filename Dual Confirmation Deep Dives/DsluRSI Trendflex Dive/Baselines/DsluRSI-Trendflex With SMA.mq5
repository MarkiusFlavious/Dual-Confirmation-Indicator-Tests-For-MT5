...
input ushort DsluRSITolerance = 3; // Dslu of RSI Lookback Tolerance
input ushort TrendflexTolerance = 3; // Trendflex Lookback Tolerance

input group "SMA Input"
input int inpSMAperiod = 20; // Period
input ENUM_APPLIED_PRICE inpSMAprice = PRICE_CLOSE; // Applied Price
input ushort SMADsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort SMATrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   SMAHandle = iMA(_Symbol,Timeframe,inpSMAperiod,0,MODE_SMA,inpSMAprice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(SMAHandle,0,1,2,MovingAverageValues);
   ArrayReverse(MovingAverageValues);
   ...
   if ((DsluRSIColourValues[0] == 1) && ((DsluRSIColourValues[1] == 0) || (DsluRSIColourValues[1] == 2)))
   {
    if (TrendflexFastValues[0] > TrendflexSlowValues[0])
    {
     ...
     {
      if (closePrice1 > MovingAverageValues[0])
      {
   ...
   // On Baseline Cross
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    if ((DsluRSIColourValues[0] == 1) && (TrendflexFastValues[0] > TrendflexSlowValues[0]))
    {
     if ((DsluRSILookback(SMADsluRSITolerance) == lookback_pass) && (TrendflexLookback(SMATrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(SMAHandle,0,1,1,MovingAverageValue);
   ...
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     {
      if ((tradeSignal == sell_signal) || (closePrice < MovingAverageValue[0]))
      {
     ...
     {
      if ((tradeSignal == buy_signal) || (closePrice > MovingAverageValue[0]))
      {
       if (trade.PositionClose(posTicket))
       {
        inTrade = false;
