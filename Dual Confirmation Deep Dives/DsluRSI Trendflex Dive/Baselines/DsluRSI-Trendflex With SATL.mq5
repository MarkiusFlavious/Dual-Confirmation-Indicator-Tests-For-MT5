...
input group "SATL Inputs"
input int inpSATLShift = 0; // Horizontal shift of the average in bars 
input ushort SATL_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort SATL_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"satl.ex5",inpSATLShift);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   if ((closePrice1 > BaselineValues[0]) && (closePrice2 < BaselineValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(SATL_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(SATL_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
