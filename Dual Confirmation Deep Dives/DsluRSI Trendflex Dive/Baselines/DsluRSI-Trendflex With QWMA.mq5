...
input group "QWMA Inputs"
input ENUM_TIMEFRAMES inpQWMATimeframe = PERIOD_CURRENT; // Time frame
input int inpQWMAperiod = 25; // MA period
input double inpQWMASpeed = 2; // MA "speed"
input enPrices inpQWMAPrice = pr_close; // Average price
input chgColor inpQWMAColorOn = chg_onLevel; // Color change on :
input int inpQWMAFlPeriod = 25; // Period for finding floating levels
input double inpQWMAFlUp = 90; // Upper level %
input double inpQWMAFlDown = 10; // Lower level %
input ushort QWMA_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort QWMA_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"QWMA.ex5",inpQWMATimeframe,inpQWMAperiod,inpQWMASpeed,inpQWMAPrice,inpQWMAColorOn,inpQWMAFlPeriod,inpQWMAFlUp,inpQWMAFlDown,false,false,false,false,false,false,true);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(BaselineHandle,3,1,2,BaselineValues);
   ArrayReverse(BaselineValues);
   
   double BaselineColourValues[];
   CopyBuffer(BaselineHandle,4,1,2,BaselineColourValues);
   ArrayReverse(BaselineColourValues);
   ...
   if ((DsluRSIColourValues[0] == 1) && ((DsluRSIColourValues[1] == 0) || (DsluRSIColourValues[1] == 2)))
   {
    if (TrendflexFastValues[0] > TrendflexSlowValues[0])
    {
     ...
     {
      if ((closePrice1 > BaselineValues[0]) && (BaselineColourValues[0] == 1))
      {
   ...
   if ((DsluRSIColourValues[0] == 2) && ((DsluRSIColourValues[1] == 0) || (DsluRSIColourValues[1] == 1)))
   {
    if (TrendflexFastValues[0] < TrendflexSlowValues[0])
    {
     if (TrendflexLookback(TrendflexTolerance) == lookback_pass)
     {
      if ((closePrice1 < BaselineValues[0]) && (BaselineColourValues[0] == 2))
      {
   ...
   // On Baseline Cross
   if (((closePrice1 > BaselineValues[0]) && (closePrice2 < BaselineValues[1])) && (BaselineColourValues[0] == 1))
   {
    if ((DsluRSIColourValues[0] == 1) && (TrendflexFastValues[0] > TrendflexSlowValues[0]))
    {
     if ((DsluRSILookback(QWMA_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(QWMA_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
   ...
   if (((closePrice1 < BaselineValues[0]) && (closePrice2 > BaselineValues[1])) && (BaselineColourValues[0] == 2))
   {
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(BaselineHandle,3,1,1,BaselineValue);
   
   double BaselineColourValue[];
   CopyBuffer(BaselineHandle,4,1,1,BaselineColourValue);
   ...
   if (inTrade == true)
   {
    if (PositionSelectByTicket(posTicket))
    {
     ...
     {
      if ((tradeSignal == sell_signal) || (closePrice < BaselineValue[0]) || (BaselineColourValue[0] == 0))
      {
     ...
     {
      if ((tradeSignal == buy_signal) || (closePrice > BaselineValue[0]) || (BaselineColourValue[0] == 0))
      {
       if (trade.PositionClose(posTicket))
       {
        inTrade = false;
