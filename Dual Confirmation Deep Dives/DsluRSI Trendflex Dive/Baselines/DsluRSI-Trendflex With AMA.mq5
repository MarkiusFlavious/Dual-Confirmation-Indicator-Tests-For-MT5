...
input group "AMA Input"
input int inpAMAperiod = 20; // AMA Period
input int inpFastEMAperiod = 2; // Fast EMA Period
input int inpSlowEMAperiod = 30; // Slow EMA Period
input ENUM_APPLIED_PRICE inpAMAprice = PRICE_CLOSE; // Applied Price
input ushort AMADsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort AMATrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   AMAHandle = iAMA(_Symbol,Timeframe,inpAMAperiod,inpFastEMAperiod,inpSlowEMAperiod,0,inpAMAprice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(AMAHandle,0,1,2,MovingAverageValues);
   ...
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(AMADsluRSITolerance) == lookback_pass) && (TrendflexLookback(AMATrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(AMAHandle,0,1,1,MovingAverageValue);
