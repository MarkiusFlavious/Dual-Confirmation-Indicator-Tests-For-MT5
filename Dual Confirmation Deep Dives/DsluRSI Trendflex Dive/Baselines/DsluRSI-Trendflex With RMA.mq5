...
input group "RMA Inputs"
input int inpRMAPeriod = 14; // Period
input ENUM_APPLIED_PRICE inpRMAPrice = PRICE_CLOSE; // Price
input ushort RMA_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort RMA_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"RMA.ex5",inpRMAPeriod,inpRMAPrice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(BaselineHandle,0,1,2,MovingAverageValues);
   ...
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(RMA_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(RMA_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(BaselineHandle,0,1,1,MovingAverageValue);
