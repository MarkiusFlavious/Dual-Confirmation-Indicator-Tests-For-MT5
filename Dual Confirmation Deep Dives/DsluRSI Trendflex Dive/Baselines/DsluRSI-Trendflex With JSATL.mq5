...
input group "JSATL Inputs"
input uint inpJSATLLength = 5; // Smoothing Depth
input int inpJSATLPhase = 100; // Phase -100 to 100
input Applied_price inpJSATLAppliedPrice = PRICE_CLOSE_; // Applied Price
input int inpJSATLShift = 0; // Horizontal Shift
input int inpJSATLPriceShift = 0; // Vertical Shift in Points
input ushort JSATL_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort JSATL_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"JSatl.ex5",inpJSATLLength,inpJSATLPhase,inpJSATLAppliedPrice,inpJSATLShift,inpJSATLPriceShift);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   if ((closePrice1 > BaselineValues[0]) && (closePrice2 < BaselineValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(JSATL_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(JSATL_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
