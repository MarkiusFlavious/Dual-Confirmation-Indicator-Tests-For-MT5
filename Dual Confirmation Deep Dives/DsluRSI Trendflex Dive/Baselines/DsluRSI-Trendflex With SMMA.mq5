...
input group "SMMA Input"
input int inpSMMAperiod = 20; // Period
input ENUM_APPLIED_PRICE inpSMMAprice = PRICE_CLOSE; // Applied Price
input ushort SMMADsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort SMMATrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   SMMAHandle = iMA(_Symbol,Timeframe,inpSMMAperiod,0,MODE_SMMA,inpSMMAprice);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   CopyBuffer(SMMAHandle,0,1,2,MovingAverageValues);
   ...
   if ((closePrice1 > MovingAverageValues[0]) && (closePrice2 < MovingAverageValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(SMMADsluRSITolerance) == lookback_pass) && (TrendflexLookback(SMMATrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
...
void PositionCheckModify(enTradeSignal tradeSignal)
  {
   ...
   CopyBuffer(SMMAHandle,0,1,1,MovingAverageValue);
