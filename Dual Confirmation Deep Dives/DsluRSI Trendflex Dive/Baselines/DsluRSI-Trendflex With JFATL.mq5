...
input group "JFATL Inputs"
input int JMALength = 5;  // Depth of the JMA smoothing                   
input int JMAPhase = 100; // JMA smoothing parameter -100 to 100
...
input ushort JFATL_DsluRSITolerance = 5; // Dslu of RSI Lookback Tolerance
input ushort JFATL_TrendflexTolerance = 5; // Trendflex Lookback Tolerance
//+------------------------------------------------------------------+
//| Handles                                                          |
//+------------------------------------------------------------------+
...
int OnInit()
  {
   ...
   BaselineHandle = iCustom(_Symbol,Timeframe,"jfatl.ex5",JMALength,JMAPhase,AppliedPrice,FATLShift,PriceShift);
...
enConfirmationSignal CheckConfirmationIndicators()
  {
   ...
   if ((closePrice1 > BaselineValues[0]) && (closePrice2 < BaselineValues[1]))
   {
    ...
    {
     if ((DsluRSILookback(JFATL_DsluRSITolerance) == lookback_pass) && (TrendflexLookback(JFATL_TrendflexTolerance) == lookback_pass))
     {
      return confirmation_buy;
     }
    }
